...
const ChatSidebar = ({ 
  ...
  const [openGroups, setOpenGroups] = useState(() => 
    Object.keys(groupedConversations).length > 0 ? [Object.keys(groupedConversations)[0]] : []
  );
  ...
  return (
    <div className={`transition-all duration-300 ease-in-out bg-gray-50 flex flex-col border-r border-gray-200 ${isOpen ? 'w-64' : 'w-0'} overflow-hidden`}>
        ...
        <AnimatePresence>
          {Object.entries(groupedConversations).map(([groupTitle, convos]) => (
            <div key={groupTitle} className="mb-2">
              <button onClick={() => toggleGroup(groupTitle)} className="w-full flex justify-between items-center text-xs font-bold text-gray-400 uppercase p-1 rounded hover:bg-gray-200">
                <span>{groupTitle}</span>
                <FaChevronDown className={`transition-transform duration-200 ${openGroups.includes(groupTitle) ? 'rotate-180' : ''}`} />
              </button>
              <AnimatePresence>
                      ...
                      {convos.map((convo) => (
                        <div key={convo.id} className="relative group flex items-center">
                          ...
                          >
                            ...
                            <p className={`text-xs truncate ${activeConversationId === convo.id ? 'text-gray-300' : 'text-gray-400'}`}>
                              {formatDistanceToNow(new Date(convo.id), { addSuffix: true, locale: id })}
                            </p>
                          </button>
                          <button
