...
const groupConversationsByDate = (conversations) => {
  const now = new Date();
  const groups = { "Hari Ini": [], "Kemarin": [], "7 Hari Terakhir": [], "Bulan Ini": [], "Lebih Lama": [] };
  conversations.forEach(convo => {
    ...
    else if (isWithinInterval(convoDate, { start: subDays(now, 7), end: now })) groups["7 Hari Terakhir"].push(convo);
    else if (isWithinInterval(convoDate, { start: startOfMonth(now), end: now })) groups["Bulan Ini"].push(convo);
...
export const useChat = () => {
  ...
  const handleFeedback = (messageIndex, feedbackType) => {
    setConversations(prev => prev.map(convo => {
      if (convo.id === activeConversationId) {
        ...
        if (newMessages[messageIndex].feedback === null) newMessages[messageIndex].feedback = feedbackType;
  ...
  const groupedAndFilteredConversations = useMemo(() => {
    const filtered = conversations.filter(convo => convo.title.toLowerCase().includes(searchTerm.toLowerCase()));
